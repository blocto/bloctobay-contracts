...
pub fun main(sortBy: UInt64, asc: Bool, offset: Int, limit: Int): DipslayResult {
    ...
    if sortBy == 0 {
        listingIds = Marketplace.getCollectionListingIDsByTime(nftType: Type<@MotoGPPack.NFT>())
    } else if sortBy == 1 {
        listingIds = Marketplace.getCollectionListingIDsByPrice(nftType: Type<@MotoGPPack.NFT>())
    }
